<!--
  ...
  Displays tasks completed per day using D3.js bar chart with week navigation.
  Follows UNIX philosophy: single responsibility for daily task count visualization.
-->
...
<script lang="ts">
  ...
  function drawChart() {
    ...
    const maxTasks = d3.max(chartData, d => d.tasksCompleted) || 5;
    ...
    g.append('g')
      ...
      .call(d3.axisLeft(yScale)
        ...
        .ticks(Math.min(maxTasks, 5))
      );

    // Add bars
    g.selectAll('.tasks-bar')
      .data(chartData)
      .enter().append('rect')
      .attr('class', 'tasks-bar')
      .attr('x', d => xScale(d.date)!)
      .attr('width', xScale.bandwidth())
      .attr('y', d => yScale(d.tasksCompleted))
      .attr('height', d => innerHeight - yScale(d.tasksCompleted))
      .attr('fill', 'var(--nord14)')
      .attr('rx', 4)
      .style('opacity', 0.8)
    ...
    g.selectAll('.bar-label')
      ...
      .attr('y', d => d.tasksCompleted > 0 ? yScale(d.tasksCompleted) - 5 : innerHeight - 5)
      .attr('text-anchor', 'middle')
      .style('font-size', 'var(--font-size-xs)')
      .style('fill', 'var(--text-secondary)')
      .text(d => d.tasksCompleted > 0 ? d.tasksCompleted : '');
  ...
  function getTooltip(): TooltipSelection | null {
    ...
    let tooltip = bodySelection.select('.tasks-chart-tooltip') as TooltipSelection;
    if (tooltip.empty()) {
      tooltip = bodySelection
        .append('div')
        .attr('class', 'tasks-chart-tooltip')
        .style('opacity', 0)
...
<div class="chart-wrapper">
  <h3 class="chart-title">Tasks Completed Per Day</h3>
  <WeekNavigator
    {weekOffset}
    {hasNextWeek}
    {hasPrevWeek}
  ...
  <div class="chart-container" bind:this={chartContainer}>
    {#if currentWeekData.length === 0 || currentWeekData.every(d => d.minutes === 0 && d.tasksCompleted === 0)}
      <div class="empty-state">
        ...
        <p class="empty-description">No completed tasks for this week</p>
      </div>
    {/if}
  </div>
</div>
...
<style>
  ...
  /* Chart styling */
  :global(.tasks-chart .x-axis),
  :global(.tasks-chart .y-axis) {
    color: var(--text-secondary);
    font-size: var(--font-size-xs);
  }

  :global(.tasks-chart .x-axis path),
  :global(.tasks-chart .y-axis path),
  :global(.tasks-chart .x-axis line),
  :global(.tasks-chart .y-axis line) {
    stroke: var(--border-default);
  }

  :global(.tasks-chart .grid line) {
    stroke: var(--border-subtle);
    stroke-opacity: 0.3;
  }

  :global(.tasks-chart .grid path) {
    stroke-width: 0;
  }
